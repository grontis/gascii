...
DONE 8-5: clicking anywhere in the animation pane "arms" it as the focused section; the next Ctrl+D duplicates the active frame, Delete removes it (refusing the last frame with a message), Ctrl+C copies the frame to an internal frame clipboard, and Ctrl+V inserts the copy after the active frame (selecting it). Clicking the canvas/sidebar disarms, returning the keys to their canvas meanings. An active selection or text session always outranks the section — its Ctrl+D/Delete/copy still target the selection. Paste refuses if the layer structure changed since the copy. Bugfix same day: Ctrl+V produced no event at all when the OS clipboard was empty (egui-winit only synthesizes a Paste event from non-empty clipboard text), so frame copy now also writes the frame's text export to the OS clipboard — which additionally means a copied frame pastes as text into other apps.
...
DONE 8-5: onion skinning removed entirely — the timeline's Onion checkbox and depth steppers, the `O` shortcut, the tint-rendering code, and its session state are all gone (the help overlay and shortcut-claim tables updated with them). The canvas decorator that carried it survives as PlaybackRenderer, keeping exactly the playback-display behavior: painting the played frame, suppressing editing overlays while playing, and honoring layer visibility.
