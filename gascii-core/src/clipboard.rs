...
impl CellPatch {
    ...
    pub fn from_external_text(text: &str, fg: Rgba, bg: Rgba) -> (CellPatch, usize) {
        ...
        let width = lines
            .iter()
            .map(|l| l.chars().count().min(max_width))
            .max()
            .unwrap_or(0) as u16;
...
mod tests {
    ...
    fn from_region_captures_exactly_the_regions_cells() {
        ...
        assert_eq!(
            patch.cells.iter().map(|c| c.ch).collect::<Vec<_>>(),
            vec!['a', 'b', 'c', 'd']
        );
    }
    ...
    fn round_trips_region_to_patch_to_cells_for_a_1x1_region() {
        ...
        assert_eq!(
            patch.cells,
            vec![cell('Q', Rgba(1, 2, 3, 255), Rgba(4, 5, 6, 255))]
        );
    }
    ...
    fn from_external_text_splits_lines_and_pads_short_rows_with_blank() {
        let (patch, dropped) =
            CellPatch::from_external_text("ab\nc", Rgba::WHITE, Rgba::TRANSPARENT);
        assert_eq!(dropped, 0);
        assert_eq!(patch.width, 2);
        assert_eq!(patch.height, 2);
    ...
    fn from_external_text_drops_wide_and_combining_characters_and_counts_them() {
        ...
        assert_eq!(
            dropped, 2,
            "the emoji and the combining mark must both be rejected"
        );
        let chars: Vec<char> = patch.cells.iter().map(|c| c.ch).collect();
        assert_eq!(
            chars,
            vec!['a', ' ', 'b', ' ', 'c'],
            "rejected chars leave their cell Blank"
        );
    }
    ...
    fn from_external_text_strips_crlf_line_endings_without_widening_or_rejecting() {
        ...
        assert_eq!(
            patch.height, 3,
            "trailing CRLF yields a final empty row, same as trailing \\n"
        );
        let chars: Vec<char> = patch.cells.iter().map(|c| c.ch).collect();
    ...
    fn from_external_text_clamps_a_line_over_65535_chars_without_panicking() {
        ...
        let (patch, dropped) =
            CellPatch::from_external_text(&long_line, Rgba::WHITE, Rgba::TRANSPARENT);
        assert_eq!(
            patch.width,
            Document::MAX_WIDTH,
            "width must clamp to the document's max, not wrap"
        );
        assert_eq!(patch.height, 1);
        assert_eq!(
            patch.cells.len(),
            Document::MAX_WIDTH as usize,
            "buffer must be sized to the clamped width"
        );
        assert_eq!(
            dropped,
            70_000 - Document::MAX_WIDTH as usize,
            "every char beyond the clamp is counted as dropped"
        );
    }
    ...
    fn from_external_text_clamps_more_than_max_height_lines_without_panicking() {
        ...
        assert_eq!(
            patch.height,
            Document::MAX_HEIGHT,
            "height must clamp to the document's max, not wrap"
        );
        assert_eq!(patch.width, 1);
        assert_eq!(
            dropped,
            2000 - Document::MAX_HEIGHT as usize,
            "every dropped line's char(s) are counted"
        );
    }
    ...
    fn from_external_text_clamps_both_dimensions_at_once_with_no_oversized_allocation() {
        ...
        let bomb: String = std::iter::repeat_with(|| "a".repeat(5000))
            .take(3000)
            .collect::<Vec<_>>()
            .join("\n");
        let (patch, _dropped) =
            CellPatch::from_external_text(&bomb, Rgba::WHITE, Rgba::TRANSPARENT);
        assert_eq!(patch.width, Document::MAX_WIDTH);
        assert_eq!(patch.height, Document::MAX_HEIGHT);
        assert_eq!(
            patch.cells.len(),
            Document::MAX_WIDTH as usize * Document::MAX_HEIGHT as usize
        );
    }
