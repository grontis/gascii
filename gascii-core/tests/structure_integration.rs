...
fn a_floating_selection_must_be_flushed_before_save_or_the_move_is_silently_lost_from_the_saved_file(
    ...
    assert_ne!(
        doc, unflushed_snapshot,
        "the drop must actually change the document relative to the unflushed state"
    );

    let json = save_string(&doc);
    let loaded =
        load_str(&json).expect("a document with a dropped selection move must save and reload");
...
fn after_save_and_reload_the_dropped_moves_edit_is_baked_in_and_has_no_undo_entry_of_its_own() {
    ...
    assert_eq!(
        loaded2, loaded,
        "undoing the one post-load edit must restore exactly the loaded (post-drop) state"
    );
    assert!(!history2.can_undo());
}
...
fn rectangle_drawn_over_an_existing_line_then_undone_restores_the_lines_original_glyph_at_the_crossing_cells(
    ...
    assert_eq!(
        doc, after_line,
        "undo of the rectangle must restore the exact pre-rectangle state byte-for-byte"
    );
    assert_eq!(
        doc.cell(0, 5, 2).unwrap().ch,
        '│',
        "the crossing cell must revert to the line's glyph, not Blank"
    );
    assert_eq!(doc.cell(0, 5, 8).unwrap().ch, '│');
}
...
fn fill_on_a_1024x1024_document_from_a_corner_is_exactly_one_undo_entry_and_round_trips_byte_exact()
{
    ...
    let edit =
        commit_edit(resp).expect("a full-canvas fill from a blank corner must commit an edit");
    history.apply(&mut doc, edit);

    assert!(
        doc.layers()[0].cells().iter().all(|c| c.ch == '#'),
    ...
    assert_eq!(
        doc, before,
        "undo of a full-canvas fill must restore byte-exact blank state"
    );
    assert!(
        !history.can_undo(),
        "a full-canvas fill must be exactly one undo entry"
    );
...
fn single_point_rectangle_degenerates_the_same_way_a_single_point_line_does() {
    ...
    let gascii_core::Edit::Cells(cells) =
        commit_edit(resp).expect("a single-point rectangle must still commit one cell")
    else {
...
fn rectangle_spanning_the_documents_full_extent_produces_exact_corner_glyphs_with_no_out_of_bounds_cells(
    ...
    let resp = rect.update(ToolEvent::Release, &tctx, &doc);
    let gascii_core::Edit::Cells(cells) = commit_edit(resp).expect("expected a committed edit")
    else {
        panic!("expected a Cells edit");
    };
    let by_pos: std::collections::HashMap<(u16, u16), char> =
    ...
    for c in &cells {
        assert!(
            c.x < 6 && c.y < 6,
            "no cell of a rectangle exactly at the document's own extent may fall out of bounds"
        );
    }
}
...
fn copying_a_region_and_pasting_it_elsewhere_reproduces_identical_glyphs_and_colors_via_the_internal_clipboard_path(
    ...
    let cells = [
        (
            ...
            Cell {
                ch: 'a',
                fg: Rgba(10, 20, 30, 255),
                bg: Rgba(40, 50, 60, 255),
            },
        ),
        (
            (1, 0),
            Cell {
                ch: 'b',
                fg: Rgba(11, 21, 31, 255),
                bg: Rgba(41, 51, 61, 255),
            },
        ),
        (
            (0, 1),
            Cell {
                ch: 'c',
                fg: Rgba(12, 22, 32, 255),
                bg: Rgba(42, 52, 62, 255),
            },
        ),
        (
            (1, 1),
            Cell {
                ch: 'd',
                fg: Rgba(13, 23, 33, 255),
                bg: Rgba(43, 53, 63, 255),
            },
        ),
    ];
...
fn round_tripping_a_copy_through_the_external_text_path_preserves_glyphs_but_resets_colors_to_the_active_ones(
    ...
    assert_eq!(
        roundtripped.cells[0].fg, active_fg,
        "external paste must use the currently active fg, not the copied cell's original color"
    );
    assert_ne!(
        roundtripped.cells[0].fg, original.cells[0].fg,
        "the external path must not smuggle the original color through plain text"
    );
...
fn pasting_a_stamp_that_hangs_off_the_document_edge_clips_the_overhanging_cells_with_no_source_to_blank(
    ...
    let resp = sel.update(ToolEvent::Commit, &tctx, &doc);
    let gascii_core::Edit::Cells(cells) = commit_edit(resp).expect("expected a clipped write edit")
    else {
        panic!("expected a Cells edit");
    };
    assert_eq!(
        cells.len(),
        4,
        "only the 2x2 in-bounds portion of the 3x3 stamp survives the clip"
    );
    for c in &cells {
        assert!(c.x < 5 && c.y < 5);
...
fn two_lines_drawn_in_separate_strokes_crossing_each_other_join_into_a_cross_and_undo_peels_back_one_stroke_at_a_time(
    ...
    assert_eq!(
        doc.cell(0, 5, 5).unwrap().ch,
        '┼',
        "two independently-committed lines crossing must union into a full cross"
    );
...
fn a_rectangle_corner_landing_on_an_existing_rectangle_corner_unions_into_a_four_way_junction() {
    ...
    assert_eq!(
        doc.cell(0, 6, 6).unwrap().ch,
        '┘',
        "sanity: rect1's bottom-right corner"
    );
    ...
    assert_eq!(
        doc.cell(0, 6, 6).unwrap().ch,
        '┼',
        "two rectangle corners landing on the same cell must union into a full cross junction"
    );
}
...
fn one_action_one_undo_entry_across_fill_rectangle_line_selection_move_delete_and_paste() {
    ...
    assert!(
        !history.can_undo(),
        "exactly six entries expected — no extra hidden entries left over"
    );
...
fn a_no_op_attempt_of_each_new_tool_action_commits_none_and_never_reaches_history() {
    ...
    assert!(matches!(
        fill.update(ToolEvent::Release, &blank_tctx, &doc),
    ...
    let mut rect = Rectangle::new();
    assert!(matches!(
        rect.update(ToolEvent::Release, &any_tctx, &doc),
        ToolResponse::Commit(None)
    ));
    let mut line = Line::new();
    assert!(matches!(
        line.update(ToolEvent::Release, &any_tctx, &doc),
        ToolResponse::Commit(None)
    ));
    ...
    assert!(matches!(
        sel3.update(ToolEvent::Commit, &any_tctx, &doc),
        ToolResponse::Commit(None)
    ));

    assert!(
        !history.can_undo(),
        "none of these no-op attempts may ever have reached History::apply"
    );
}
