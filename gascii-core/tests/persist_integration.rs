...
fn full_lifecycle_round_trip_through_mixed_tool_pipeline_preserves_document_and_resets_history() {
    ...
    assert_eq!(
        doc.cell(0, 5, 5).unwrap().bg,
        Rgba(11, 22, 33, 255),
        "sanity: selective stroke preserved bg pre-save"
    );
    ...
    assert_eq!(
        ...
        '#',
        "sanity: bg-only erase left the glyph alone"
    );
    ...
    let loaded = load_str(&json)
        .expect("a document built entirely through the tool pipeline must load back");
    ...
    assert!(
        !history2.can_undo(),
        "a freshly loaded document must start with empty undo history"
    );
    ...
    assert_ne!(
        doc2, loaded,
        "sanity: the post-load edit changed the document"
    );
    ...
    assert_eq!(
        doc2, loaded,
        "undo of the one post-load edit must restore the exact loaded state"
    );
    assert!(
        !history2.can_undo(),
        "fresh history must have exactly the one entry — no phantom pre-load edits to undo further"
    );
}
...
fn round_trip_preserves_box_drawing_glyphs() {
    ...
    assert_eq!(
        ...
        '│',
        "non-ASCII content must survive a round trip unstripped"
    );
}
...
fn text_tool_at_the_far_corner_of_a_1024_square_document_stops_at_both_edges_and_commits_one_edit()
{
    ...
    assert!(
        matches!(after_enter, ToolResponse::Idle),
        "Enter at the bottom-right corner must go inert, not wrap"
    );
    ...
    assert_eq!(
        cells.len(),
        2,
        "only the two in-bounds chars typed before hitting the edge may commit"
    );
    let mut by_pos: Vec<((u16, u16), char)> =
...
fn interleaved_non_overlapping_text_bursts_survive_a_full_undo_redo_round_trip_then_a_third_overwrite(
    ...
    assert!(
        ...
        "second undo must revert burst 1, restoring a fully blank document"
    );
    ...
    assert!(
        !history.can_redo(),
        "a new commit after the round trip must clear any redo stack"
    );
