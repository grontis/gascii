...
use gascii_core::{
    builtin_ramps, load_str, resize_document, save_string, BrushShape, Buildup, Cell, CellPatch,
    DensityBrush, DensityMode, Document, Edit, Fixed, History, Pencil, PlaneMask, ResizeAnchor,
    ResizeError, Rgba, SelectionTool, TextTool, Tool, ToolCtx, ToolEvent, ToolResponse,
};
...
fn resize_after_flushing_a_pending_text_burst_bakes_the_burst_in_before_growing_the_document() {
    ...
    assert_eq!(
        doc.cell(0, 0, 0),
        Some(&Cell::BLANK),
        "sanity: an uncommitted burst never touches doc directly"
    );
    ...
    assert_eq!(
        ...
        'H',
        "the flushed burst's content must survive the grow"
    );
    assert_eq!(doc.cell(0, 1, 0).unwrap().ch, 'i');
...
fn resize_after_flushing_a_floating_selection_drop_operates_on_the_post_drop_document_not_a_stale_pre_drop_one(
    ...
    assert_eq!(
        doc.cell(0, 2, 2),
        Some(&Cell::BLANK),
        "sanity: the source is vacated by the drop"
    );
    ...
    assert_eq!(
        doc.cell(0, 2, 2),
        Some(&Cell::BLANK),
        "resize must see the post-drop document, not a stale pre-drop one"
    );
    assert_eq!(doc.cell(0, 3, 3), Some(&content));
    assert_eq!(doc.cell(0, 5, 5), Some(&content));
}
...
fn resize_then_save_then_load_round_trip_preserves_the_new_extent_and_content() {
    ...
    assert_eq!(
        loaded, doc,
        "the loaded document must be byte-for-byte identical to the resized one"
    );
}
...
fn resize_then_undo_then_redo_then_save_round_trips_the_redone_post_resize_state() {
    ...
    assert_eq!(
        doc, after_grow,
        "redo must restore exactly the post-resize state"
    );

    let loaded = load_str(&save_string(&doc)).unwrap();
    assert_eq!(
        loaded, after_grow,
        "a save taken after redo must reflect the redone (grown) extent, not the undone one"
    );
}
...
fn resize_one_past_the_cap_is_cleanly_rejected_while_exactly_at_the_cap_applies_as_one_undo_entry()
{
    ...
    assert!(
        !history.can_undo(),
        "a rejected resize request must never reach History"
    );

    let edit = resize_document(&doc, Document::MAX_WIDTH, Document::MAX_HEIGHT, start())
        .unwrap()
        .unwrap();
    history.apply(&mut doc, edit);
    assert_eq!(
        (doc.width, doc.height),
        (Document::MAX_WIDTH, Document::MAX_HEIGHT)
    );
    ...
    assert_eq!(
        (doc.width, doc.height),
        (10, 10),
        "exactly one undo must fully restore the pre-cap-resize extent"
    );
}
...
fn shrinking_to_1x1_then_growing_back_preserves_only_the_top_left_anchor_cell_not_the_rest_of_the_original_content(
    ...
    assert_eq!(
        doc.cell(0, 0, 0),
        Some(&cell('a')),
        "the top-left anchor cell survives both operations"
    );
    assert_ne!(
        doc, original,
        "content elsewhere must NOT be resurrected by growing back to the original size"
    );
    assert_eq!(
        doc.cell(0, 4, 4),
        Some(&Cell::BLANK),
        "content lost to the shrink is genuinely gone, not restored"
    );
    ...
    assert_eq!(
        doc, original,
        "undoing both resizes in order must restore the exact original 5x5 content"
    );
    assert!(!history.can_undo());
}
...
fn a_same_size_resize_attempt_mid_session_produces_no_history_entry_and_a_single_undo_still_reverts_only_the_real_edit(
    ...
    assert_eq!(
        resize_document(&doc, 6, 6, start()).unwrap(),
        None,
        "same-size resize is a no-op with no Edit produced"
    );
...
fn buildup_stroke_over_mixed_on_ramp_off_ramp_and_blank_cells_commits_as_one_edit_and_undoes_byte_exact(
    ...
    assert_eq!(
        doc.cell(0, 0, 0).unwrap().ch,
        'y',
        "on-ramp 'x' (index 1) must advance to index 2"
    );
    assert_eq!(
        doc.cell(0, 1, 0).unwrap().ch,
        'w',
        "an off-ramp glyph must land on step 0"
    );
    assert_eq!(
        doc.cell(0, 2, 0).unwrap().ch,
        'w',
        "a Blank cell, off-ramp for a ramp with no space, must also land on step 0"
    );
    ...
    assert_eq!(
        doc, original,
        "one undo must restore all three cells to their exact pre-stroke values"
    );
    assert!(
        !history.can_undo(),
        "the whole mixed-content stroke must be exactly one undo entry"
    );
}
...
fn fixed_intensity_with_a_glyph_only_mask_writes_the_ramp_glyph_and_its_text_color_but_preserves_the_bg(
    ...
    assert_eq!(
        result.ch, '@',
        "Fixed(1.0) over a 10-char ramp lands on the last (darkest) glyph"
    );
    assert_eq!(
        result.fg,
        Rgba(255, 0, 0, 255),
    ...
    assert_eq!(
        result.bg, existing.bg,
        "glyph-only mask must preserve the cell's original bg"
    );
}
...
fn a_brush_stroke_survives_a_save_load_round_trip_and_a_second_stroke_continues_buildup_from_the_persisted_state(
    ...
    let loaded = load_str(&save_string(&doc))
        .expect("a document touched by the density brush must save and reload");
    assert_eq!(
        loaded.cell(0, 2, 2).unwrap().ch,
        'a',
        "the brushed glyph must round-trip exactly"
    );
...
fn buildup_on_the_built_in_space_first_ascii_ramp_touching_a_blank_cell_advances_past_index_zero_to_index_one(
    ...
    assert_eq!(
        doc.cell(0, 1, 1),
        Some(&Cell::BLANK),
        "sanity: the target cell is genuinely Blank before the stroke"
    );

    let ascii_shading = builtin_ramps()
        .into_iter()
        .find(|r| r.name == "ASCII shading")
        .unwrap();
    assert_eq!(
        ascii_shading.chars[0], ' ',
        "sanity: the built-in ramp this test targets is genuinely space-first"
    );
...
fn one_action_one_undo_entry_across_pencil_brush_and_resize_interleaved_walks_undo_redo_through_mixed_variant_history(
    ...
    for i in 1..snapshots.len() {
        assert_ne!(
            snapshots[i],
            snapshots[i - 1],
            "action {i} must actually have changed the document"
        );
    }
    ...
    assert!(
        !history.can_undo(),
        "exactly six entries expected across the mixed Cells/Resize sequence"
    );
...
fn paste_after_a_resize_still_clips_correctly_against_the_new_post_resize_bounds() {
    ...
    for c in &cells {
        assert!(
            c.x < 4 && c.y < 4,
            "no cell may land outside the post-resize extent"
        );
    }
}
