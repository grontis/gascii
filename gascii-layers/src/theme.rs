//! A minimal, vendored copy of the host's color tokens — the fields `widgets::button` and the
//! layer-row painting actually read. Duplicated rather than threaded through `PluginHost`,
//! mirroring `gascii-density-brush::theme`/`gascii-anim::theme`'s own precedent exactly (a pure
