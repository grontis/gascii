...
fn scratch_dir(tag: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!(
        "gascii_write_atomic_test_{tag}_{}",
        std::process::id()
    ));
...
fn sized_slots_are_distinct_and_in_range() {
    ...
    for kind in [
        ...
        assert_eq!(
            sized_slot(kind),
            None,
            "{kind:?} must not have a stamp slot"
        );
    }
}
...
fn derived_stamp_slots_are_dense_and_distinct_and_match_sized_tool_count() {
    ...
    assert_eq!(
        slots, expected,
        "stamp slots must be exactly 0..sized_tool_count(), each once"
    );
}
...
fn stamp_slot_is_some_exactly_when_sized() {
    for d in tools() {
        assert_eq!(
            d.stamp_slot.is_some(),
            d.sized,
            "{:?}: stamp_slot.is_some() must match sized",
            d.name
        );
    }
}
...
fn tools_table_lists_every_kind_exactly_once() {
    ...
    for kind in ALL_KINDS {
        ...
        assert_eq!(
            count, 1,
            "{kind:?} appears {count} times in the tool registry"
        );
    }
}
...
fn capability_fields_match_expected_for_every_kind() {
    for kind in ALL_KINDS {
        ...
        assert_eq!(
            d.holds_session, expected_holds_session,
            "{kind:?}: holds_session"
        );
        assert_eq!(d.shows_hover, expected_shows_hover, "{kind:?}: shows_hover");
        assert_eq!(
            d.stamps_glyph, expected_stamps_glyph,
            "{kind:?}: stamps_glyph"
        );
        assert_eq!(
        ...
        assert_eq!(
            d.kiosk_visible, expected_kiosk_visible,
            "{kind:?}: kiosk_visible"
        );
        assert_eq!(
            d.pressure_sizeable, expected_pressure_sizeable,
            "{kind:?}: pressure_sizeable"
        );
        assert_eq!(
            d.wants_ctx_patch, expected_wants_ctx_patch,
            "{kind:?}: wants_ctx_patch"
        );
    }
}
...
fn validate_unique_tool_names_rejects_a_duplicate_name() {
    ...
    rows.push(dupe);
    let err =
        validate_unique_tool_names(&rows).expect_err("a duplicate tool name must be an error");
    assert!(
        err.contains(dupe.name),
        "error must name the duplicated tool: {err}"
    );
}
...
fn every_registered_descriptor_carries_non_empty_metadata() {
    for d in PLUGINS {
        ...
        assert!(
            !d.version.is_empty(),
            "descriptor {:?} has an empty version",
            d.id
        );
    }
}
...
fn merge_plugin_row_carries_the_given_plugin_slot_index_verbatim() {
    ...
    for slot in [0usize, 1, 4, 7] {
        ...
        assert_eq!(
            row.plugin_slot,
            Some(slot),
            "merge_plugin_row must not hardcode a plugin_slot index"
        );
    }
}
...
fn a_fresh_apps_brush_row_plugin_slot_resolves_to_the_live_instance_that_registered_brush() {
    ...
    assert!(
        app.plugins[slot]
            .as_any_mut()
            .downcast_mut::<gascii_density_brush::BrushPlugin>()
            .is_some(),
...
fn two_independent_gascii_apps_never_share_brush_plugin_state() {
    ...
    app1.brush_plugin_mut()
        .set_density_mode(gascii_core::DensityMode::Buildup(gascii_core::Buildup));
    app1.brush_plugin_mut().set_pressure_enabled(true);

    assert_eq!(
        app2.brush_plugin_mut().active_ramp(),
        0,
        "app2 must start at Brush's own default ramp, not app1's mutated one"
    );
    assert!(
        matches!(
            app2.brush_plugin_mut().density_mode(),
            gascii_core::DensityMode::Fixed(_)
        ),
        "app2 must not see app1's Buildup mode"
    );
    assert!(
        !app2.brush_plugin_mut().pressure_enabled(),
        "app2 must not see app1's pressure opt-in"
    );
...
fn every_plugins_panel_hook_is_a_true_no_op_for_the_real_builtin_list() {
    ...
    assert_eq!(
        app.doc, before,
        "no plugin's panel hook may mutate the document"
    );
}
...
impl Plugin for BottomPanelDouble {
    fn panel(
        ...
        egui::Panel::bottom("test_timeline_double")
            .exact_size(50.0)
            .show(ui, |ui| {
...
fn anim_panel_shows_only_the_slim_reopen_bar_while_frame_count_is_one() {
    ...
    let layers = PLUGINS
        .iter()
        .position(|d| d.id == gascii_layers::DESCRIPTOR.id)
        .expect("gascii-layers is registered");
    ...
    assert!(
        shrink > 0.0,
        "the reopen bar must claim real space — it's the timeline's discoverability affordance"
    );
...
fn anim_panel_claims_space_once_a_second_frame_exists() {
    let mut app = GasciiApp::headless();
    let edit = gascii_core::add_frame(
        &app.doc,
        1,
        gascii_core::Frame::blank(app.doc.width, app.doc.height),
    )
    .unwrap();
...
fn plugin_panel_outcome_edits_are_applied_through_apply_edit() {
    ...
    impl Plugin for EditOutcomeDouble {
        fn panel(
            ...
            _ui: &mut egui::Ui,
            _kiosk: bool,
            host: &dyn gascii_plugin_api::PluginHost,
        ) -> gascii_plugin_api::PanelOutcome {
            let idx = host.document().active_frame();
            gascii_plugin_api::PanelOutcome {
                edits: vec![gascii_core::Edit::SetFrameDuration {
                    index: idx,
                    before: None,
                    after: Some(50),
                }],
    ...
    assert_eq!(
        app.doc.resolved_frame_duration_ms(0),
        Some(50),
        "the PanelOutcome's edit must reach the document"
    );
    assert!(
        app.history.can_undo(),
        "the edit must have gone through History::apply, not bypassed it"
    );
    app.request_undo();
    assert_eq!(
        app.doc.resolved_frame_duration_ms(0),
...
fn plugin_panel_outcome_set_active_frame_flushes_pending_sessions_before_switching() {
    ...
    app.slots[Binding::L.ix()]
        .tool
        .update(ToolEvent::Press { x: 0, y: 0 }, &tctx, &app.doc);
    app.slots[Binding::L.ix()]
        .tool
        .update(ToolEvent::Char('a'), &tctx, &app.doc);
...
fn plugin_panel_outcome_set_active_layer_flushes_pending_sessions_before_switching() {
    ...
    assert_eq!(
        app.active_layer, 1,
        "sanity: adding a layer makes it the active one"
    );
...
fn plugin_panel_outcome_set_loop_playback_writes_the_document_field_directly_without_history() {
    let mut app = GasciiApp::headless();
    assert!(
        app.doc.loop_playback,
        "sanity: a fresh document defaults to looping"
    );
    ...
    assert!(
        !app.doc.loop_playback,
        "the PanelOutcome's request must reach Document.loop_playback"
    );
    assert_eq!(
        app.history.can_undo(),
        can_undo_before,
        "a plain field write must never create an undo entry"
    );
}
...
fn plugin_panel_outcome_set_default_frame_duration_writes_the_document_field_directly_without_history(
    ...
    assert_eq!(
        app.doc.frame_duration_ms, 250,
        "the PanelOutcome's request must reach Document.frame_duration_ms"
    );
    assert_eq!(
        app.history.can_undo(),
        can_undo_before,
...
fn toggling_loop_playback_marks_the_document_dirty_even_though_it_is_never_an_edit() {
    ...
    assert!(
        app.is_dirty(),
        "toggling Loop must mark the document dirty even with an untouched undo stack"
    );
    assert!(
        !app.history.can_undo(),
        "sanity: still no undo entry — this must be caught by the snapshot comparison, not History"
    );
}
...
fn toggling_the_default_frame_duration_marks_the_document_dirty() {
    ...
    assert!(
        app.is_dirty(),
        "changing the default frame duration must mark the document dirty"
    );
}
...
fn saving_resets_the_saved_session_meta_snapshot_so_the_document_reads_clean_again() {
    ...
    let path = std::env::temp_dir().join(format!(
        "gascii_dirty_snapshot_save_test_{}.gascii",
        std::process::id()
    ));
    ...
    assert!(
        !app.is_dirty(),
        "a successful save must reset the session-meta snapshot alongside the undo marker"
    );
}
...
fn opening_a_document_resets_the_saved_session_meta_snapshot_to_the_loaded_documents_own_values() {
    ...
    let edit = gascii_core::add_frame(
        &fresh,
        1,
        gascii_core::Frame::blank(fresh.width, fresh.height),
    )
    .unwrap();
    ...
    let path = std::env::temp_dir().join(format!(
        "gascii_dirty_snapshot_open_test_{}.gascii",
        std::process::id()
    ));
    ...
    assert!(
        !app.is_dirty(),
        "a fresh open must reset the snapshot to the loaded document's own values"
    );
    assert!(
        !app.doc.loop_playback,
        "sanity: the loaded document's own loop_playback was actually applied"
    );
}
...
fn create_new_document_resets_active_frame_and_active_layer() {
    let mut app = GasciiApp::headless();
    let add_frame = gascii_core::add_frame(
        &app.doc,
        1,
        gascii_core::Frame::blank(app.doc.width, app.doc.height),
    )
    ...
    assert_eq!(
        app.active_frame, 0,
        "a fresh document must reset the frame cursor"
    );
    assert_eq!(
        app.active_layer, 0,
        "a fresh document must reset the layer cursor"
    );
}
...
fn open_path_resets_active_frame_and_active_layer() {
    ...
    assert_eq!(
        app.active_frame, 0,
        "opening a document must reset the frame cursor"
    );
    assert_eq!(
        app.active_layer, 0,
        "opening a document must reset the layer cursor"
    );
}
...
fn toggling_loop_playback_raises_the_close_confirm_veto_exactly_like_a_real_edit_would() {
    ...
    assert!(
        !app.history.can_undo(),
        "sanity: still no undo entry — this must go through the snapshot comparison"
    );
    ...
    assert_eq!(
        app.confirm,
        Some(PendingConfirm::CloseApp),
        "a Loop-only dirty document must veto the close exactly like an edited one"
    );
}
...
fn plugin_panel_outcome_error_surfaces_through_last_error() {
    ...
    impl Plugin for ErrorOutcomeDouble {
        fn panel(
            ...
            gascii_plugin_api::PanelOutcome {
                error: Some("add frame: exceeds the 256 maximum".to_string()),
                ..Default::default()
            }
        }
        fn as_any_mut(&mut self) -> &mut dyn std::any::Any {
    ...
    assert_eq!(
        app.last_error_text(),
        Some("add frame: exceeds the 256 maximum")
    );
}
...
fn error_flash_expires_for_the_status_bar_but_persists_for_dialog_reads() {
    ...
    app.last_error = Some(crate::app::ErrorFlash {
        text: "boom".to_string(),
        at: now,
    });
    ...
    assert_eq!(text, "boom");
    assert!(
        left <= crate::app::ERROR_FLASH_TTL,
        "time-left is bounded by the TTL"
    );

    let after_ttl = now + crate::app::ERROR_FLASH_TTL;
    assert!(
        app.error_flash(after_ttl).is_none(),
        "at/past the TTL the status bar stops showing it"
    );
    assert_eq!(
        app.last_error_text(),
        Some("boom"),
        "the dialog-inline read must not expire"
    );

    app.flash_error("again");
    assert!(
        app.error_flash(std::time::Instant::now()).is_some(),
        "a re-raise restarts the clock"
    );
}
...
fn plugin_panel_outcome_with_both_an_edit_and_an_error_applies_the_edit_and_still_surfaces_the_error(
    ...
    assert_eq!(
        app.doc.resolved_frame_duration_ms(0),
        Some(50),
        "the succeeding half of a multi-op outcome must still apply"
    );
    assert_eq!(
        app.last_error_text(),
        Some("duplicate frame: exceeds the 256 maximum"),
        "the failing half must still surface"
    );
}
...
fn plugin_outcome_originated_duplicate_frame_edit_lands_in_history_and_undoes_identically_to_the_menu_path(
    ...
    let ctx = egui::Context::default();
    let _ = ctx.run_ui(egui::RawInput::default(), |ui| {
        app_plugin.run_plugin_panels(ui, false)
    });
    ...
    assert_eq!(
        app_plugin.doc, app_menu.doc,
        "undo must restore both paths to an identical document"
    );
    assert_eq!(
        app_plugin.doc.frame_count(),
        1,
        "undo must fully reverse the plugin-originated duplicate"
    );
}
...
fn plugin_panel_outcome_that_both_adds_a_frame_and_switches_to_it_in_one_pass_resolves_against_the_post_edit_document(
    ...
    impl Plugin for AddAndSwitchDouble {
        fn panel(
            ...
            let doc = host.document();
            let edit =
                gascii_core::add_frame(doc, 1, gascii_core::Frame::blank(doc.width, doc.height))
                    .unwrap();
            gascii_plugin_api::PanelOutcome {
                edits: vec![edit],
                properties: vec![gascii_plugin_api::DocProperty::ActiveFrame(1)],
    ...
    assert_eq!(
        app.doc.frame_count(),
        2,
        "the edit half of the outcome must have landed"
    );
...
fn plugin_panel_outcome_originated_delete_edit_undoes_to_a_byte_exact_prior_document() {
    ...
    impl Plugin for DeleteFrameDouble {
        fn panel(
            ...
            match gascii_core::remove_frame(doc, doc.active_frame()) {
                Ok(edit) => gascii_plugin_api::PanelOutcome {
                    edits: vec![edit],
                    ..Default::default()
                },
    ...
    assert_eq!(
        app.doc.frame_count(),
        1,
        "the plugin-outcome-originated delete must have applied"
    );

    app.request_undo();
    assert_eq!(
        app.doc, before_delete,
        "undo must byte-exactly restore the prior 2-frame document"
    );
}
...
fn request_undo_is_refused_while_a_plugin_blocks_editing() {
    ...
    assert_eq!(
        app.doc.cell_at(0, 0, 0, 0).unwrap().ch,
        'Z',
        "the refused undo must leave the edit in place"
    );
...
fn a_press_inside_the_animation_panel_arms_the_frames_section_and_one_outside_disarms() {
    ...
    let ctx = egui::Context::default();
    let _ = ctx.run_ui(press(egui::Pos2::new(500.0, 790.0)), |ui| {
        app.run_plugin_panels(ui, false)
    });
    assert!(
        app.frames_section_armed,
        "a press inside the bottom timeline panel must arm"
    );

    let ctx2 = egui::Context::default();
    let _ = ctx2.run_ui(press(egui::Pos2::new(500.0, 100.0)), |ui| {
        app.run_plugin_panels(ui, false)
    });
    assert!(
        !app.frames_section_armed,
        "a press up in canvas territory must disarm"
    );
}
...
fn ctrl_d_duplicates_the_active_frame_while_the_frames_section_is_armed() {
    ...
    raw.events
        .push(key_event(egui::Key::D, egui::Modifiers::COMMAND));
    let _ = ctx.run_ui(raw, |ui| app.handle_keys(ui));

    assert_eq!(
        app.doc.frame_count(),
        2,
        "Ctrl+D in frames mode must duplicate the frame"
    );
...
fn an_active_selection_outranks_the_armed_frames_section_for_ctrl_d() {
    ...
    app.select_all();
    assert!(
        app.keyboard_owner().is_some(),
        "sanity: the marquee owns the keyboard"
    );
    ...
    assert_eq!(
        app.doc.frame_count(),
        1,
        "the selection's duplicate wins — no frame is created"
    );
    assert!(
        app.selection_slot().is_some(),
        "the selection session survives"
    );
}
...
fn delete_removes_the_active_frame_while_armed_and_refuses_the_last_frame() {
    ...
    let delete_press = || {
        let mut raw = egui::RawInput::default();
        raw.events
            .push(key_event(egui::Key::Delete, egui::Modifiers::NONE));
        raw
    };
    let ctx = egui::Context::default();
    ...
    assert_eq!(
        app.doc.frame_count(),
        1,
        "Delete in frames mode must remove the active frame"
    );
...
fn copy_then_paste_inserts_a_frame_copy_after_the_active_one_while_armed() {
    ...
    assert!(
        app.frame_clipboard.is_some(),
        "Ctrl+C in frames mode must fill the frame clipboard"
    );
    ...
    assert!(
        out.platform_output
            .commands
            .iter()
            .any(|c| matches!(c, egui::OutputCommand::CopyText(t) if !t.is_empty())),
    ...
    assert_eq!(
        app.doc.frame_count(),
        2,
        "Ctrl+V in frames mode must insert the copied frame"
    );
    assert_eq!(app.active_frame, 1, "the pasted frame is selected");
    assert_eq!(
        app.doc.cell_at(1, 0, 0, 0).unwrap().ch,
        'Q',
        "the pasted frame carries the copied content"
    );
}
...
fn paste_frame_refuses_an_empty_clipboard_and_a_layer_mismatched_frame() {
    ...
    assert!(app
        .last_error_text()
        .unwrap()
        .contains("no frame has been copied"));
    ...
    assert_eq!(
        app.doc.frame_count(),
        1,
        "a layer-mismatched frame must not be inserted"
    );
...
fn switch_active_frame_mid_pencil_drag_does_not_silently_commit_the_pending_stroke_to_either_frame()
{
    ...
    assert_eq!(
        app.active_frame, 1,
        "the switch itself must still take effect"
    );
    ...
    assert_eq!(
        app.doc.cell_at(1, 0, 0, 0).unwrap().ch,
        ' ',
        "nor onto the frame just switched to"
    );
    ...
    if let ToolResponse::Commit(Some(edit)) =
        app.slots[Binding::L.ix()]
            .tool
            .update(ToolEvent::Release, &tctx2, &app.doc)
    {
    ...
    assert_eq!(
        app.doc.cell_at(1, 0, 0, 0).unwrap().ch,
        'Z',
        "the eventual release commits onto whichever frame is active when it fires"
    );
    assert_eq!(
        app.doc.cell_at(0, 0, 0, 0).unwrap().ch,
        ' ',
        "the origin frame is left untouched by a release that fires after the switch"
    );
}
...
fn add_frame_via_menu_reports_the_max_frames_boundary_with_a_specific_readable_message() {
    ...
    assert_eq!(
        app.doc.frame_count(),
        Document::MAX_FRAMES,
        "a rejected add must not change frame_count"
    );
    assert_eq!(
        app.last_error_text(),
        Some("add frame: exceeds the 256 maximum")
...
fn add_frame_menu_item_duplicates_the_active_frame_and_flushes_first() {
    ...
    app.slots[Binding::L.ix()]
        .tool
        .update(ToolEvent::Press { x: 1, y: 0 }, &tctx, &app.doc);
    app.slots[Binding::L.ix()]
        .tool
        .update(ToolEvent::Char('X'), &tctx, &app.doc);
    app.acquire_keyboard(Binding::L);
    ...
    assert_eq!(
        app.doc.frame_count(),
        2,
        "Add Frame must duplicate into a second frame"
    );
    assert_eq!(
        app.doc.cell_at(0, 0, 1, 0).unwrap().ch,
        'X',
        "the pending burst must be flushed before duplicating"
    );
    assert_eq!(
        app.doc.cell_at(1, 0, 0, 0).unwrap().ch,
        'D',
        "the duplicate must carry the source frame's content"
    );
    assert_eq!(
        app.doc.cell_at(1, 0, 1, 0).unwrap().ch,
        'X',
        "the duplicate must carry the just-flushed burst too"
    );
    assert!(app.last_error.is_none());
}
...
fn add_frame_via_menu_selects_the_newly_added_frame_and_undo_returns_the_cursor() {
    ...
    assert_eq!(
        app.active_frame, 1,
        "the new frame becomes the working frame"
    );
    assert_eq!(app.doc.active_frame(), 1);
    ...
    assert_eq!(
        app.active_frame, 0,
        "undo returns the cursor to the source frame"
    );
}
...
fn add_frame_draw_switch_frame_and_undo_twice_restores_the_single_frame_starting_state() {
    ...
    app.slots[Binding::L.ix()]
        .tool
        .update(ToolEvent::Press { x: 2, y: 2 }, &r, &app.doc);
    if let ToolResponse::Commit(Some(edit)) =
        app.slots[Binding::L.ix()]
            .tool
            .update(ToolEvent::Release, &r, &app.doc)
    {
        app.apply_edit(edit, Some(Binding::L));
    }
    ...
    app.request_undo();
    assert_eq!(
        app.doc, starting_doc,
        "two undos must fully restore the pre-Add-Frame document"
    );
}
...
fn build_renderer_folds_every_plugins_wrap_renderer_in_registration_order() {
    ...
    assert_eq!(
        *log.borrow(),
        vec!["a", "b", "c"],
        "fold order must match plugin-list order"
    );
}
...
fn disabling_the_plugin_owning_a_bound_tool_rebinds_to_pencil_and_preserves_its_stamp() {
    ...
    assert_eq!(
        ...
        ToolKind::Pencil,
        "L must fall back to Pencil at disable time"
    );
    assert_eq!(
        app.slots[Binding::L.ix()].stamps[slot].size,
        7,
        "the stamp array is per-binding, not per-bound-tool — disable must not touch it"
    );
    ...
    assert_eq!(
        app.slot(Binding::L).kind,
        ToolKind::Pencil,
        "re-enable must not auto-rebind"
    );
    ...
    assert_eq!(
        app.slots[Binding::L.ix()].stamps[slot].size,
        7,
        "rebinding after the cycle must see the same stamp size"
    );
}
...
fn a_disabled_plugins_tool_cannot_be_bound() {
    ...
    assert_eq!(
        app.slot(Binding::L).kind,
        ToolKind::Pencil,
        "binding a disabled plugin's tool must be refused"
    );
    ...
    assert_eq!(
        app.slot(Binding::L).kind,
        BRUSH_KIND,
        "the same bind must succeed once re-enabled"
    );
}
...
fn a_disabled_plugins_tick_is_skipped_and_its_first_tick_after_reenable_sees_the_resume_flag() {
    ...
    assert_eq!(
        *log.borrow(),
        vec![false],
        "an ordinary enabled tick carries no resume flag"
    );
    ...
    assert_eq!(
        *log.borrow(),
        vec![false],
        "a disabled plugin must not be ticked at all"
    );
    ...
    assert_eq!(
        *log.borrow(),
        vec![false, true],
        "the first tick after re-enable must see resumed_after_suppression"
    );
    ...
    assert_eq!(
        *log.borrow(),
        ...
        "the latch is one-shot — the second tick is ordinary again"
    );
}
...
fn disabling_a_plugin_removes_its_panel_from_the_layout() {
    ...
    assert!(
        enabled_rect.height() < baseline_rect.height(),
        "sanity: the enabled double's bottom panel must claim space"
    );
    assert_eq!(
        disabled_rect, baseline_rect,
        "disabling must reclaim the panel's space down to the exact baseline rect"
    );
}
...
fn disabling_the_layers_plugin_with_multiple_layers_does_not_reset_active_layer_or_break_rendering()
{
    ...
    assert_eq!(
        app.active_layer, 1,
        "disabling the plugin must not reset active_layer"
    );
    assert_eq!(app.doc.active_layer(), 1);
    assert_eq!(
        app.doc, before,
        "disabling a plugin must never touch the document"
    );

    let ctx = egui::Context::default();
    fonts::install_fonts(&ctx);
    let _ = ctx.run_ui(egui::RawInput::default(), |_ui| {});
    let out = ctx.run_ui(raw_input_with_screen(300.0, 300.0), |ui| {
        crate::canvas::show(ui, &mut app, false)
    });
    let seeded_color =
        egui::Color32::from_rgba_unmultiplied(top_bg.0, top_bg.1, top_bg.2, top_bg.3);
    let count = out
        .shapes
        .iter()
    ...
    assert_eq!(
        count, 1,
        "the host-owned composite must still include layer 1's content with gascii-layers disabled"
    );
}
...
fn toggling_a_plugin_rebuilds_the_renderer_excluding_disabled_wrappers() {
    let mut app = GasciiApp::headless();
    let log = std::rc::Rc::new(std::cell::RefCell::new(Vec::new()));
    let i = app.push_plugin_double(Box::new(TaggingPlugin {
        tag: "double",
        log: log.clone(),
    }));
    let brush = tool_def(BRUSH_KIND)
        .plugin_slot
        .expect("Brush is plugin-sourced");
    ...
    assert_eq!(
        *log.borrow(),
        vec!["double"],
        "a rebuild triggered by toggling ANOTHER plugin must still wrap the enabled double"
    );
    ...
    assert!(
        log.borrow().is_empty(),
        "the rebuild after disabling the double must not call its wrap_renderer"
    );
    ...
    assert_eq!(
        *log.borrow(),
        vec!["double"],
        "re-enabling must fold the double back into the renderer chain"
    );
}
...
fn plugin_state_survives_a_disable_enable_cycle() {
    ...
    assert_eq!(
        ...
        1,
        "the live instance must be retained across the toggle, state and all"
    );
}
...
fn the_tool_letter_of_a_disabled_plugins_tool_is_left_unconsumed() {
    ...
    assert!(
        key_still_there,
        "handle_keys must not consume a disabled tool's shortcut key"
    );
    assert_eq!(
        app.slot(Binding::L).kind,
        ToolKind::Pencil,
        "and must not rebind L off its default"
    );
}
...
fn opening_the_plugins_dialog_counts_as_a_modal() {
    ...
    assert!(
        app.modal_open(),
        "the Plugins dialog must register with modal_open()"
    );
}
...
fn anim_plugin_enabled_follows_the_anim_toggle_and_ignores_other_plugins() {
    ...
    app.set_plugin_enabled(brush, false);
    assert!(
        app.anim_plugin_enabled(),
        "another plugin's toggle must not affect the Animation menu gate"
    );
    ...
    assert!(
        !app.anim_plugin_enabled(),
        "disabling gascii-anim must hide the Animation menu"
    );
...
fn every_kind_builds_a_tool_with_an_empty_pending_overlay() {
    for kind in ALL_KINDS {
        ...
        assert!(
            ...
            "{kind:?} starts with a non-empty overlay"
        );
    }
}
...
fn stamps_are_per_slot_so_sizing_rs_eraser_never_resizes_ls() {
    ...
    assert_eq!(
        slots[Binding::L.ix()].stamp().size,
        1,
        "L's Eraser was resized by R's"
    );
}
...
fn pressure_override_drives_tool_ctx_size_without_touching_the_slots_configured_size() {
    ...
    assert_eq!(
        tctx.size, 2,
        "the live stroke's footprint follows the pressure override"
    );
    assert_eq!(
        app.slots[Binding::L.ix()].stamps[brush_slot].size,
        10,
    ...
    assert_ne!(
        r_tctx.size, 2,
        "pressure only overrides the stroke-owning binding"
    );
...
fn tool_ctx_injects_extra_context_only_for_a_plugin_tool_that_wants_it() {
    ...
    assert_eq!(
        brush_ctx.ramp, expected_ramp,
        "Brush's tool_ctx.ramp must follow the live plugin's active ramp"
    );

    app.bind(Binding::L, ToolKind::Pencil);
    let pencil_ctx = crate::canvas::tool_ctx(&app, Binding::L);
    assert!(
        pencil_ctx.ramp.is_empty(),
        "a non-plugin tool must get the inert default, not Brush's ramp"
    );
}
...
fn a_full_brush_stroke_through_the_app_commits_a_glyph_from_the_plugins_active_ramp() {
    ...
    app.slots[Binding::L.ix()]
        .tool
        .update(ToolEvent::Press { x: 2, y: 2 }, &tctx, &app.doc);
    if let ToolResponse::Commit(Some(edit)) =
        app.slots[Binding::L.ix()]
            .tool
            .update(ToolEvent::Release, &tctx, &app.doc)
    ...
    app.slots[Binding::L.ix()]
        .tool
        .update(ToolEvent::Press { x: 3, y: 3 }, &pencil_ctx, &app.doc);
    if let ToolResponse::Commit(Some(edit)) =
        app.slots[Binding::L.ix()]
            .tool
            .update(ToolEvent::Release, &pencil_ctx, &app.doc)
    {
        app.apply_edit(edit, Some(Binding::L));
    }
...
fn brush_bound_only_to_r_while_l_holds_a_different_tool_still_resolves_through_the_plugin() {
    ...
    assert_eq!(
        r_ctx.ramp, expected_ramp,
        "R's tool_ctx must resolve through the plugin even though L holds a different tool"
    );
    let l_ctx = crate::canvas::tool_ctx(&app, Binding::L);
    assert!(
        l_ctx.ramp.is_empty(),
        "L (Pencil) must not see Brush's ramp just because R holds Brush"
    );
}
    ...
    for fullscreen in [false, true] {
        ...
        match app.brush_plugin_mut().density_mode() {
            gascii_core::DensityMode::Fixed(gascii_core::Fixed(level)) => {
                assert!(
                    (level - 0.5).abs() < 1e-4,
                    "fullscreen={fullscreen}: expected Fixed(0.5)"
                )
            }
...
fn active_frame_defaults_to_zero_and_tool_ctx_follows_it() {
    ...
    assert_eq!(
        tctx.frame, 1,
        "tool_ctx's frame must follow a non-default active_frame too"
    );
}
...
fn apply_edit_syncs_doc_active_frame_from_app_active_frame_before_applying() {
    ...
    assert_eq!(
        app.doc.active_frame(),
        1,
        "apply_edit must sync doc's active-frame cursor from app.active_frame"
    );
}
...
fn apply_edit_syncs_doc_active_layer_from_app_active_layer_before_applying() {
    ...
    app.apply_edit(cell_edit, None);
    assert_eq!(
        app.doc.active_layer(),
        1,
        "apply_edit must sync doc's active-layer cursor from app.active_layer"
    );
}
...
fn undoing_an_add_frame_moves_the_docs_cursor_and_app_active_frame_follows() {
    ...
    assert_eq!(
        app.doc.active_frame(),
        1,
        "the inserted frame becomes the active cursor"
    );
    assert_eq!(
        app.active_frame, 1,
        "apply_edit's doc -> app resync must follow the move"
    );
    ...
    assert_eq!(
        app.doc.active_frame(),
        0,
        "undo restores doc's pre-insert cursor"
    );
    assert_eq!(
        app.active_frame, 0,
        "app.active_frame must follow doc's cursor back down after undo"
    );
}
...
fn a_stroke_undo_redo_save_load_cycle_through_the_app_still_produces_a_plain_v1_file() {
    ...
    app.stroke_owner = None;
    assert_eq!(
        app.doc.cell(0, 2, 2).unwrap().ch,
        '#',
        "sanity: the stroke committed"
    );
    ...
    assert_eq!(
        app.doc.cell(0, 2, 2).unwrap().ch,
        ' ',
        "sanity: undo reverted the stroke"
    );
    app.request_redo();
    assert_eq!(
        app.doc.cell(0, 2, 2).unwrap().ch,
        '#',
        "sanity: redo restored it"
    );

    assert_eq!(
        app.doc.frame_count(),
        1,
        "the shipped app never leaves frame_count() == 1"
    );
    assert_eq!(
        app.active_frame, 0,
        "the shipped app never moves active_frame off 0"
    );
    ...
    assert_eq!(
        value["version"], 1,
        "a single-frame session must be tagged version 1, the pre-frames version"
    );
...
fn f11_toggles_fullscreen_even_during_an_active_text_session() {
    ...
    app.slots[Binding::L.ix()]
        .tool
        .update(ToolEvent::Char('h'), &tctx, &app.doc);
    app.acquire_keyboard(Binding::L);
    ...
    assert!(
        sent_toggle,
        "F11 must toggle fullscreen even while a Text session is active"
    );
}
...
fn startup_window_state_is_forced_exactly_once() {
    ...
    assert!(
        sent_windowed,
        "the first frame must pin the window state even if eframe restored fullscreen"
    );
    assert!(
        app.startup_fullscreen.is_none(),
        "the forced state is one-shot"
    );
    ...
    assert!(
        !sent_again,
        "later frames must never re-force the window state"
    );
}
...
fn paste_target_prefers_an_existing_selection_binding_over_rebinding() {
    ...
    assert_eq!(
        paste_target(Pencil, Selection),
        Binding::R,
        "should not clobber L's binding"
    );
    assert_eq!(
        paste_target(Selection, Selection),
        Binding::L,
        "L wins when both qualify"
    );
    assert_eq!(
        paste_target(Pencil, Pencil),
        Binding::L,
        "falls back to L, never R"
    );
}
...
fn a_strokes_commit_repins_the_other_bindings_live_session() {
    ...
    app.slots[Binding::L.ix()]
        .tool
        .update(ToolEvent::Press { x: 0, y: 0 }, &l, &app.doc);
    app.slots[Binding::L.ix()]
        .tool
        .update(ToolEvent::Char('A'), &l, &app.doc);

    // R: a pencil stroke commits '#' into (0,0), beneath the burst.
    app.active_glyph = '#';
    let r = crate::canvas::tool_ctx(&app, Binding::R);
    app.slots[Binding::R.ix()]
        .tool
        .update(ToolEvent::Press { x: 0, y: 0 }, &r, &app.doc);
    if let ToolResponse::Commit(Some(edit)) =
        app.slots[Binding::R.ix()]
            .tool
            .update(ToolEvent::Release, &r, &app.doc)
...
fn a_slot_holding_a_session_is_the_only_one() {
    ...
    assert_eq!(
        app.selection_slot(),
        Some(Binding::R),
        "two selections would be ambiguous"
    );
}
...
fn rebinding_releases_only_its_own_keyboard_claim() {
    ...
    assert_eq!(
        app.keyboard_owner,
        Some(Binding::R),
        "rebinding L muted R's session"
    );

    app.set_tool(Binding::R, ToolKind::Fill);
    assert_eq!(
        app.keyboard_owner, None,
        "rebinding R should release its own claim"
    );
}
...
fn ctrl_c_response_first_press_requests_a_normal_close() {
    assert_eq!(
        ctrl_c_response(1, 0, false),
        Some(CtrlCResponse::RequestClose)
    );
}
...
fn tool_shortcut_reachable_only_gates_eyedropper_and_only_while_fullscreen() {
    for kind in ALL_KINDS {
        assert!(
            tool_shortcut_reachable(kind, false),
            "{kind:?}: every shortcut works windowed"
        );
        let expected = kind != ToolKind::Eyedropper;
        assert_eq!(
            tool_shortcut_reachable(kind, true),
            expected,
            "{kind:?}: fullscreen gating must affect only Eyedropper"
        );
    }
}
...
fn flush_slot_never_releases_keyboard_ownership() {
    ...
    assert_eq!(
        app.keyboard_owner(),
        Some(Binding::L),
        "flush must never release the keyboard"
    );
    assert!(
...
fn a_mid_stroke_flush_commits_the_float_so_the_dirty_check_sees_it() {
    ...
    app.slots[Binding::L.ix()]
        .tool
        .accept_stamp(patch, (3, 3), &app.doc);
    app.acquire_keyboard(Binding::L);
    ...
    assert_eq!(
        app.doc.cell(0, 3, 3).unwrap().ch,
        'x',
        "the float must commit at its current spot"
    );
    assert!(
        app.is_dirty(),
        "the close-confirm dirty check must see the committed float"
    );
}
...
fn end_session_commits_pending_work_even_for_the_stroke_owning_binding() {
    ...
    assert_eq!(
        app.stroke_owner,
        Some(Binding::L),
        "sanity: the press is still held"
    );
    let tctx = crate::canvas::tool_ctx(&app, Binding::L);
    app.slots[Binding::L.ix()]
        .tool
    ...
    assert_eq!(
        app.doc.cell(0, 0, 0).unwrap().ch,
        'h',
        "the held-press burst must commit, not vanish"
    );
...
fn ctrl_c_then_delete_workflow_survives_a_flush() {
    ...
    app.slots[Binding::L.ix()]
        .tool
        .update(ToolEvent::Press { x: 1, y: 1 }, &tctx, &app.doc);
    app.slots[Binding::L.ix()]
        .tool
        .update(ToolEvent::Drag { x: 2, y: 2 }, &tctx, &app.doc);
    app.slots[Binding::L.ix()]
        .tool
        .update(ToolEvent::Release, &tctx, &app.doc);
    app.acquire_keyboard(Binding::L);
...
fn mid_typing_structural_flush_does_not_let_the_next_letter_rebind_the_tool() {
    ...
    assert_eq!(
        owner_kind,
        Some(ToolKind::Text),
        "a structural flush must not release the keyboard mid-burst"
    );
    assert!(
            suppresses_tool_shortcuts(owner_kind),
...
fn a_strokes_commit_mid_gesture_repins_a_flushed_idle_slots_masked_plane() {
    ...
    assert_eq!(
        app.doc.cell(0, 0, 0).unwrap().ch,
        'A',
        "L's burst committed under R's live stroke"
    );
    ...
    app.slots[Binding::R.ix()]
        .tool
        .update(ToolEvent::Drag { x: 2, y: 0 }, &r, &app.doc);
...
fn copy_paste_drag_then_save_mid_drag_commits_the_float_and_the_session_stays_interactive() {
    ...
    assert_eq!(
        app.doc.cell(0, 0, 0).unwrap().ch,
        ' ',
        "sanity: a paste floats, it doesn't write yet"
    );

    // Grab the float and drag it.
    assert!(
        crate::canvas::begin_gesture(&mut app, Binding::L, 0, 0, false, false),
        "the press on the float starts a drag"
    );
    ...
    assert_eq!(
        app.doc.cell(0, 2, 2).unwrap().ch,
        'x',
        "the float committed at its dragged position"
    );
    let saved_doc = load_str(&std::fs::read_to_string(&path).unwrap()).unwrap();
    assert_eq!(
        saved_doc.cell(0, 2, 2).unwrap().ch,
        'x',
        "the saved file reflects the dragged position"
    );
    ...
    assert_eq!(
        app.keyboard_owner(),
        Some(Binding::L),
        "the flush must not release the keyboard mid-drag"
    );
    ...
    let resp =
        app.slots[Binding::L.ix()]
            .tool
            .update(ToolEvent::Press { x: 5, y: 5 }, &tctx, &app.doc);
    assert!(
        matches!(resp, ToolResponse::Active),
        "a fresh press must start a clean marquee, not error"
    );
    assert!(
        app.slots[Binding::L.ix()]
            .tool
...
fn the_cut_workflow_copy_delete_undo_redo_preserves_content_at_every_step() {
    ...
    assert_eq!(
        app.doc.cell(0, 1, 1).unwrap().ch,
        'x',
        "copy must not itself mutate the document"
    );
    ...
    let ToolResponse::Commit(Some(edit)) = resp else {
        panic!("Delete must produce a committed edit")
    };
    app.apply_edit(edit, Some(Binding::L));
    for (x, y) in [(1u16, 1u16), (2, 2)] {
        assert_eq!(
            app.doc.cell(0, x, y),
            Some(&gascii_core::Cell::BLANK),
            "cut must blank the region"
        );
    }
...
fn copy_selection_captures_the_active_layer_not_layer_0() {
    ...
    assert_eq!(
        patch.to_text(),
        "Q",
        "the clipboard must hold the active layer's content, not layer 0's blank cell"
    );
}
...
fn cut_on_a_non_zero_active_layer_clipboard_matches_what_delete_removed() {
    ...
    assert_eq!(
        app.doc.cell(1, 1, 1),
        Some(&gascii_core::Cell::BLANK),
        "cut must remove the active layer's content"
    );
    let patch = app
        .internal_clipboard
        .as_ref()
    ...
    assert_eq!(
        patch.to_text(),
        "Q",
        "the clipboard must hold exactly what was removed from the active layer"
    );
}
...
fn redoing_the_other_bindings_stroke_resyncs_a_live_burst_preserving_its_masked_off_plane() {
    ...
    assert_eq!(
        app.doc.cell(0, 0, 0),
        Some(&gascii_core::Cell::BLANK),
        "sanity: undo reverted R's stroke"
    );
...
fn rebinding_the_other_binding_through_several_kinds_leaves_a_live_burst_untouched_then_rebinding_its_own_binding_commits_it(
    ...
    app.slots[Binding::L.ix()]
        .tool
        .update(ToolEvent::Char('h'), &l, &app.doc);
    app.acquire_keyboard(Binding::L);
    ...
    for kind in [
        ...
        assert_eq!(
            ...
            kind,
            "R must actually rebind to {kind:?}"
        );
        assert_eq!(
            app.keyboard_owner(),
            Some(Binding::L),
            "R's rebind must not touch L's session"
        );
        assert!(
            app.slots[Binding::L.ix()].tool.caret().is_some(),
    ...
    app.slots[Binding::L.ix()]
        .tool
        .update(ToolEvent::Char('i'), &l, &app.doc);
    ...
    assert_eq!(
        app.doc.cell(0, 0, 0).unwrap().ch,
        'h',
        "rebinding L must commit, not discard, the burst"
    );
...
fn opening_a_file_strands_neither_a_live_burst_nor_an_in_flight_stroke_onto_the_new_document() {
    ...
    assert!(crate::canvas::begin_gesture(
        &mut app,
        Binding::R,
        2,
        2,
    ...
    app.slots[Binding::R.ix()]
        .tool
        .update(ToolEvent::Drag { x: 3, y: 2 }, &r, &app.doc);
    assert_eq!(
        app.stroke_owner,
        Some(Binding::R),
        "sanity: R is mid-stroke"
    );
    ...
    assert_eq!(
        app.doc.cell(0, 0, 0),
        Some(&gascii_core::Cell::BLANK),
        "L's burst must not have committed"
    );
    assert_eq!(
        app.doc.cell(0, 2, 2),
        Some(&gascii_core::Cell::BLANK),
        "R's in-flight stroke must not have committed"
    );
    assert_eq!(
        app.stroke_owner, None,
        "R's in-flight stroke claim must not survive Open"
    );
    assert_eq!(
        app.keyboard_owner(),
        None,
        "L's session claim must not survive Open"
    );
    assert!(
        app.slots[Binding::L.ix()].tool.caret().is_none(),
        "L's caret must not survive Open"
    );
    assert!(
        app.slots[Binding::R.ix()].tool.pending().is_empty(),
        "R's in-flight stroke cells must not survive Open"
    );
    ...
    app.slots[Binding::L.ix()]
        .tool
        .update(ToolEvent::Press { x: 1, y: 1 }, &l2, &app.doc);
    assert!(
        app.slots[Binding::L.ix()].tool.caret().is_some(),
        "the new Text instance is interactive"
    );
}
...
fn note_recent_file_is_most_recent_first_deduplicated_and_capped_at_eight() {
    ...
    assert_eq!(
        app.recent_files[0],
        PathBuf::from("9.gascii"),
        "most recent is first"
    );
    assert_eq!(
        app.recent_files[7],
        PathBuf::from("2.gascii"),
    ...
    assert_eq!(
        app.recent_files[0], reopened,
        "re-opening moves it to the front"
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        8,
        "re-adding an existing entry does not grow the list"
    );
}
...
fn a_failed_reopen_drops_the_path_from_recent_files() {
    ...
    assert!(
        ...
        "a failed open must drop the dead entry"
    );
...
fn step_zoom_defers_into_pending_step_zoom_without_touching_the_viewport() {
    ...
    assert_eq!(
        app.viewport.zoom_step, before,
        "the viewport itself must be untouched until canvas::show applies it"
    );
}
...
fn opening_a_dialog_while_another_is_open_replaces_it() {
    ...
    assert_eq!(
        app.open_dialog,
        Some(OpenDialog::Export),
        "opening Export must replace Help, not add to it"
    );

    app.open_resize_dialog();
    assert_eq!(
        app.open_dialog,
        Some(OpenDialog::Resize),
        "opening Resize must replace Export"
    );
    ...
    assert_eq!(
        app.open_dialog,
        Some(OpenDialog::New),
        "opening New must replace Resize"
    );
}
...
fn export_trim_checkbox_toggles_between_trimmed_and_full_width_padded_rows() {
    ...
    assert_eq!(
        trimmed, "#####\nab",
        "trim must drop row 1's trailing blanks but leave the full row untouched"
    );
    assert_eq!(
        untrimmed, "#####\nab   ",
        "untrimmed must pad row 1 to the full document width"
    );
    assert_ne!(
        trimmed, untrimmed,
        "the two export paths must genuinely diverge for this document"
    );
}
...
fn a_single_frame_documents_format_list_is_exactly_text_and_png() {
    ...
    let names: Vec<&str> = export_dialog_formats(&doc)
        .iter()
        .map(|(_, label)| *label)
        .collect();
...
fn a_multi_frame_documents_format_list_offers_all_five_formats_in_order() {
    ...
    let names: Vec<&str> = export_dialog_formats(&app.doc)
        .iter()
        .map(|(_, label)| *label)
        .collect();
    assert_eq!(
...
fn snap_unavailable_export_format_only_touches_multi_frame_only_formats_at_frame_count_one() {
    for format in [
        ...
        assert_eq!(
            snap_unavailable_export_format(format, 1),
            ExportFormat::Text
        );
        assert_eq!(
            snap_unavailable_export_format(format, 2),
            format,
            "must be a no-op while still offered"
        );
    }
    for format in [ExportFormat::Text, ExportFormat::Png] {
        assert_eq!(
            snap_unavailable_export_format(format, 1),
            format,
            "an always-offered format is never snapped"
        );
    }
}
...
fn refresh_export_preview_builds_a_texture_for_every_raster_format_but_not_text_frames() {
    ...
    for format in [
        ...
        assert!(
            app.export_preview.is_some(),
            "{format:?} must build a preview texture"
        );
    }
    ...
    app.refresh_export_preview(&ctx);
    assert!(
        app.export_preview.is_none(),
        "TextFrames must not build a preview texture"
    );
}
...
fn gif_and_spritesheet_bytes_round_trip_through_write_atomic() {
    ...
    assert_eq!(
        ...
        (app.doc.width as u32 * 8, app.doc.height as u32 * 8)
    );
...
fn a_gif_export_rejected_for_too_many_frames_writes_no_file_and_leaves_the_document_untouched() {
    ...
    for _ in 1..196 {
        let edit =
            gascii_core::add_frame(&doc, doc.frame_count(), gascii_core::Frame::blank(80, 25))
                .unwrap();
        history.apply(&mut doc, edit);
    }
    ...
    match anim_export::export_gif(&doc, 16, None, None) {
        ...
        Err(e) => assert!(matches!(
            e,
            png_export::PngExportAppError::Dimensions(
                gascii_core::PngExportError::TooManyFrames { .. }
            )
        )),
    }
    ...
    assert!(
        !path.exists(),
        "a rejected export must never reach write_atomic, so no file may exist"
    );
    assert_eq!(
        doc, before,
        "a failed export must not mutate the source document"
    );
    assert_eq!(
        history.top_edit_id(),
        before_top_edit,
        "a failed export must not touch history"
    );
    let _ = std::fs::remove_dir_all(&dir);
}
...
fn a_spritesheet_export_rejected_for_a_too_large_tiled_canvas_writes_no_file_and_leaves_the_document_untouched(
    ...
    for _ in 1..4 {
        let edit = gascii_core::add_frame(
            &doc,
            doc.frame_count(),
            gascii_core::Frame::blank(1024, 512),
        )
        .unwrap();
        history.apply(&mut doc, edit);
    }
    ...
    match anim_export::export_spritesheet(&doc, 8, None, None) {
        Ok(_) => {
            panic!("a 2x2 grid of 8192x4096 tiles (~134MP) must be rejected by the spritesheet cap")
        }
        Err(e) => assert!(matches!(
            e,
            png_export::PngExportAppError::Dimensions(gascii_core::PngExportError::TooLarge { .. })
        )),
    }
...
fn a_persisted_gif_preference_meeting_a_single_frame_document_snaps_back_to_text_on_reopen() {
    ...
    assert_eq!(
        single.doc.frame_count(),
        1,
        "sanity: a fresh headless app starts single-frame"
    );
    ...
    assert_eq!(
        snapped,
        ExportFormat::Text,
        "the dialog's reopen-time guard must snap an unavailable format back to Text"
    );
}
...
fn export_png_is_byte_identical_to_manually_driving_the_frame_explicit_rasterizer_on_realistic_content(
    ...
    let (w, h, pixels) =
        png_export::rasterize_frame_rgba8(&doc, doc.active_frame(), 12, opaque_bg, None).unwrap();
    let img = image::RgbaImage::from_raw(w, h, pixels).unwrap();
    let mut via_manual_frame_explicit = Vec::new();
    img.write_to(
        &mut std::io::Cursor::new(&mut via_manual_frame_explicit),
        image::ImageFormat::Png,
    )
    .unwrap();

    assert_eq!(
        via_export_png, via_manual_frame_explicit,
        "export_png must remain byte-identical to the frame-explicit rasterization path"
    );
}
...
fn a_full_new_draw_frames_durations_save_load_export_lifecycle_round_trips_correctly() {
    ...
    let edit = gascii_core::set_frame_duration(&app.doc, 2, Some(250))
        .unwrap()
        .unwrap();
    app.apply_edit(edit, None);
    ...
    assert_eq!(
        numer / denom,
        250,
        "frame 2's reloaded duration_override must survive into the GIF's delay"
    );
    assert!(
        !gif_bytes.windows(11).any(|w| w == b"NETSCAPE2.0"),
        "the reloaded loop_playback == false must write no loop extension"
    );
    ...
    let (frame_px_w, frame_px_h) =
        gascii_core::validate_png_dimensions(loaded.width, loaded.height, 8).unwrap();
    let (sheet_w, sheet_h) =
        gascii_core::validate_spritesheet_dimensions(frame_px_w, frame_px_h, 2, 2).unwrap();
    ...
    assert!(
        found_blue_in_frame_2s_tile,
        "frame 2's own tile must contain its glyph's exact fg color somewhere"
    );
...
fn paint_add_layer_paint_hide_export_then_undo_restores_layer_0_exactly() {
    ...
    assert_eq!(
        app.active_layer, 1,
        "sanity: add_layer lands its own new layer active"
    );
    ...
    app.stroke_owner = None;
    assert_eq!(
        app.doc.cell(1, 1, 1).unwrap().ch,
        'B',
        "sanity: layer 1's paint landed"
    );
    ...
    assert!(
        text.contains('B'),
        "text export must include layer 1's glyph: {text:?}"
    );
    assert!(
        !text.contains('A'),
        "text export must exclude layer 0's now-hidden glyph: {text:?}"
    );
    ...
    let tile_pixels = |cx: u32, cy: u32| -> Vec<[u8; 4]> {
        (0..cell_px)
            .flat_map(|py| (0..cell_px).map(move |px| (px, py)))
            .map(|(px, py)| pixel_at(cx * cell_px + px, cy * cell_px + py))
            .collect()
    };
    assert!(
    ...
    assert!(
        app.doc.layer_visible(0),
        "undoing the hide must restore layer 0's visibility"
    );
    assert_eq!(
        app.doc.cell(0, 0, 0).unwrap().ch,
        'A',
        "layer 0's content must still be exactly what was painted"
    );

    app.request_undo(); // undoes layer 1's paint
    app.request_undo(); // undoes add_layer
    assert_eq!(
        app.doc, after_paint0,
        "undoing back to just after layer 0's paint must match that snapshot exactly"
    );

    app.request_undo(); // undoes layer 0's paint
    assert_eq!(
        app.doc, pristine,
        "undoing the entire sequence must restore the pristine document exactly"
    );
}
...
fn create_new_document_carries_the_dialog_background_onto_the_fresh_document() {
    ...
    assert_eq!(
        app.open_dialog, None,
        "creating the document must close the dialog"
    );
    assert!(
        !app.history.can_undo(),
        "a fresh document starts with empty history"
    );
}
...
fn clear_document_app_method_produces_one_undoable_step() {
    ...
    assert!(app.doc.layers()[0]
        .cells()
        .iter()
        .all(gascii_core::Cell::is_blank));
    assert!(app.history.can_undo());
...
fn clear_document_flushes_a_pending_text_burst_before_blanking() {
    ...
    assert_eq!(
        app.doc.cell(0, 0, 0).unwrap().ch,
        'A',
        "undo #1 restores the burst's commit"
    );
    assert!(app.history.undo(&mut app.doc));
    assert_eq!(
        app.doc.cell(0, 0, 0).unwrap().ch,
        ' ',
        "undo #2 restores the pre-burst blank state"
    );
}
...
fn clear_document_flushes_a_pending_floating_selection_stamp_before_blanking() {
    ...
    app.slots[Binding::L.ix()]
        .tool
        .accept_stamp(patch, (4, 4), &app.doc);
    ...
    assert!(app.history.undo(&mut app.doc));
    assert_eq!(
        app.doc.cell(0, 4, 4).unwrap().ch,
        'z',
        "undo #1 restores the dropped stamp"
    );
    assert!(app.history.undo(&mut app.doc));
    assert_eq!(
        app.doc.cell(0, 4, 4).unwrap().ch,
        ' ',
        "undo #2 restores the pre-drop blank state"
    );
}
...
fn clear_document_survives_an_undo_then_redo_round_trip() {
    ...
    assert_eq!(
        app.doc, before,
        "undo must restore the exact pre-Clear document"
    );
    assert!(app.history.can_redo());
    ...
    assert_eq!(
        app.doc, after_clear,
        "redo must re-apply the exact same Clear edit"
    );
    assert!(!app.history.can_redo());
}
...
fn clear_mid_stroke_resyncs_the_pending_drags_masked_off_bg_plane_to_the_post_clear_blank_state() {
    ...
    app.mask = PlaneMask {
        glyph: true,
        bg: false,
    }; // bg masked off: must always track `before`
    app.active_glyph = '#';
    app.active_fg = Rgba::WHITE;
    ...
    assert_eq!(
        app.stroke_owner,
        Some(Binding::L),
        "Clear must not itself end an in-progress stroke"
    );
    ...
    assert_eq!(
        committed.ch, '#',
        "the unmasked glyph plane still stamps the drawn glyph"
    );
    assert_ne!(
        committed.bg, old_bg,
        "the masked-off bg plane must not resurrect the pre-Clear color"
    );
    assert_eq!(
        committed.bg,
        Rgba::TRANSPARENT,
...
fn begin_gesture_always_clears_a_stale_pressure_override_regardless_of_which_binding_or_tool_set_it(
    ...
    assert_eq!(
        tctx.size,
        app.slots[Binding::R.ix()].stamps[pencil_slot].size,
...
fn toggle_fullscreen_snaps_pending_fit_only_on_the_false_to_true_transition() {
    ...
    assert!(
        !app.pending_fit,
        "exiting fullscreen must NOT re-trigger a Fit snap"
    );
}
...
fn escape_precedence_chain_first_ends_text_session_second_exits_fullscreen() {
    ...
    assert_eq!(
        app.keyboard_owner(),
        Some(Binding::L),
        "sanity: the Text session holds the keyboard"
    );
    ...
    assert!(
        !toggled_frame1,
        "frame 1's Escape must be claimed by the session, not fullscreen"
    );
    assert_eq!(
        app.keyboard_owner(),
        None,
    ...
    assert!(
        toggled_frame2,
        "frame 2's Escape, with no session left, must exit fullscreen"
    );
}
...
fn escape_does_not_exit_fullscreen_while_a_stroke_is_mid_drag() {
    ...
    assert!(
        !toggled,
        "Escape must not exit fullscreen while a stroke is mid-drag"
    );
    assert!(
        app.stroke_in_progress(),
        "the mid-drag stroke itself must be untouched"
    );
}
...
fn escape_does_not_exit_fullscreen_while_a_widget_has_focus_but_does_once_nothing_is_focused() {
    ...
    assert!(
        !toggled_frame1,
        "a focused widget must withhold Escape from the fullscreen exit"
    );
    ...
    assert!(
        toggled_frame2,
        "with nothing focused, Escape must exit fullscreen exactly as before"
    );
}
...
fn f11_mid_text_burst_leaves_the_burst_content_and_caret_fully_intact_after_toggling() {
    ...
    app.slots[Binding::L.ix()]
        .tool
        .update(ToolEvent::Char('i'), &tctx, &app.doc);
    app.acquire_keyboard(Binding::L);
    ...
    assert_eq!(
        app.keyboard_owner(),
        Some(Binding::L),
        "F11 must not release the keyboard"
    );
    assert!(
        app.slots[Binding::L.ix()].tool.caret().is_some(),
        "F11 must not clear the caret"
    );
    let tctx = crate::canvas::tool_ctx(&app, Binding::L);
    app.slots[Binding::L.ix()]
        .tool
        .update(ToolEvent::Char('!'), &tctx, &app.doc);
    app.flush_slot(Binding::L);

    assert_eq!(app.doc.cell(0, 0, 0).unwrap().ch, 'h');
    assert_eq!(app.doc.cell(0, 1, 0).unwrap().ch, 'i');
    assert_eq!(
        app.doc.cell(0, 2, 0).unwrap().ch,
        '!',
        "typing after the F11 toggle must still commit"
    );
}
...
fn pressing_t_while_fullscreen_switches_l_to_text() {
    ...
    assert_ne!(
        app.slot(Binding::L).kind,
        ToolKind::Text,
        "sanity: L starts on something else"
    );
    ...
    assert_eq!(
        app.slot(Binding::L).kind,
        ToolKind::Text,
        "T must switch L to Text while fullscreen"
    );
}
...
fn x_key_swaps_fg_and_bg_in_both_windowed_and_fullscreen_chrome() {
    for fullscreen in [false, true] {
        ...
        assert_eq!(
            app.active_fg,
            Rgba(4, 5, 6, 255),
            "fullscreen={fullscreen}: X must swap FG"
        );
        assert_eq!(
            app.active_bg,
            Rgba(1, 2, 3, 255),
            "fullscreen={fullscreen}: X must swap BG"
        );
    }
}
...
fn loading_a_second_image_over_an_existing_one_replaces_it_and_frees_the_old_texture() {
    ...
    assert_eq!(
        app.image_bg_gen, 2,
        "each load bumps the generation once, not merged into one bump"
    );
    assert!(
    ...
    assert!(
        ctx.tex_manager().read().meta(id_b).is_some(),
        "the second (current) texture must still be allocated"
    );
}
...
fn an_image_bg_gen_bump_invalidates_the_cached_export_preview_key_with_export_settings_unchanged() {
    ...
    assert!(
        key_before.is_some(),
        "sanity: a PNG-format refresh must produce a cache key"
    );
    ...
    assert_ne!(
        ...
        "an image_bg_gen bump alone must change the cache key"
    );
    assert_eq!(
...
fn pressing_t_while_windowed_rebinds_l_to_text() {
    ...
    assert_ne!(
        app.slot(Binding::L).kind,
        ToolKind::Text,
        "sanity: L doesn't already start on Text"
    );
    ...
    assert_eq!(
        app.slot(Binding::L).kind,
        ToolKind::Text,
        "T must rebind L to Text while windowed"
    );
}
...
fn close_bracket_grows_only_the_options_focused_bindings_stamp_through_handle_keys() {
    ...
    assert_eq!(
        app.slots[Binding::R.ix()].stamps[slot].size,
        2,
        "R (options_focus) must grow"
    );
    assert_eq!(
        app.slots[Binding::L.ix()].stamps[slot].size,
        1,
        "L must be untouched by R's focused key"
    );
}
...
fn a_committed_stroke_updates_recent_glyphs_exactly_for_stamps_glyph_kinds() {
    for (kind, should_note) in [(ToolKind::Pencil, true), (ToolKind::Eraser, false)] {
        ...
        assert!(
            ...
            "{kind:?}: sanity, RECENT starts empty"
        );
...
fn active_layer_resync_and_undo_redo_all_target_the_same_non_default_layer_under_adversarial_sequencing(
    ...
    assert_eq!(
        app.active_layer, 2,
        "sanity: each add_layer lands its own new layer as active"
    );
    app.mask = PlaneMask::ALL;
    ...
    if let ToolResponse::Commit(Some(edit)) =
        app.slots[Binding::R.ix()]
            .tool
            .update(ToolEvent::Release, &r_tctx, &app.doc)
    {
        app.apply_edit(edit, Some(Binding::R));
    }
    app.stroke_owner = None;
    assert_eq!(
        app.doc.cell(2, 2, 2).unwrap().ch,
        'Z',
        "sanity: R's first stroke landed on layer 2"
    );
    assert_eq!(
        app.doc.cell(0, 2, 2),
        Some(&gascii_core::Cell::BLANK),
        "sanity: layer 0 untouched so far"
    );
    ...
    app.slots[Binding::L.ix()]
        .tool
        .update(ToolEvent::Press { x: 2, y: 2 }, &l_tctx, &app.doc);
    app.active_glyph = 'A';
    let l_tctx = crate::canvas::tool_ctx(&app, Binding::L);
    app.slots[Binding::L.ix()]
        .tool
        .update(ToolEvent::Char('A'), &l_tctx, &app.doc);
    ...
    if let ToolResponse::Commit(Some(edit)) =
        app.slots[Binding::R.ix()]
            .tool
            .update(ToolEvent::Release, &r_tctx2, &app.doc)
    {
        app.apply_edit(edit, Some(Binding::R));
    }
    app.stroke_owner = None;
    assert_eq!(
        app.doc.cell(2, 2, 2).unwrap().ch,
        'Y',
        "sanity: R's second stroke committed 'Y' on layer 2"
    );
    ...
    assert_eq!(
        app.doc.cell(2, 2, 2).unwrap().ch,
        'A',
        "L's committed burst lands 'A' on layer 2"
    );
    ...
    assert_eq!(
        app.doc.cell(2, 2, 2).unwrap().ch,
        'A',
        "redo must re-land the Text commit"
    );
...
fn ctrl_c_via_event_copy_copies_the_live_selections_text_to_the_internal_clipboard() {
    ...
    let patch = app
        .internal_clipboard
        .as_ref()
        .expect("Ctrl+C must populate the internal clipboard");
    assert_eq!(
        patch.to_text(),
        "x",
        "the copied patch must hold the selected cell's glyph"
    );
}
...
fn ctrl_shift_c_via_event_copy_with_shift_held_copies_the_whole_document_as_text() {
    ...
    assert!(
        copied,
        "Ctrl+Shift+C must copy the whole document's exported text to the OS clipboard"
    );
}
...
fn copy_copy_all_and_cut_are_suppressed_while_a_widget_has_keyboard_focus() {
    ...
    assert!(
        app.internal_clipboard.is_none(),
        "a focused widget must suppress Ctrl+C/Ctrl+Shift+C"
    );
    assert!(
    ...
    assert_eq!(
        app.doc.cell(0, 1, 1).unwrap().ch,
        'x',
        "a focused widget must suppress Ctrl+X's deletion too"
    );
}
...
fn copy_and_cut_still_fire_via_event_copy_and_event_cut_while_unfocused() {
    ...
    assert_eq!(
        app.doc.cell(0, 1, 1).unwrap().ch,
        ' ',
        "an unfocused Ctrl+X must still delete the selection"
    );
}
...
fn ctrl_shift_z_via_handle_keys_fires_redo_not_undo() {
    ...
    assert!(
        !app.history.can_undo(),
        "sanity: no undo is available after the single edit was undone"
    );
    ...
    assert_eq!(
        app.doc.cell(0, 0, 0).unwrap().ch,
        '#',
        "Ctrl+Shift+Z must have redone the edit"
    );
    assert!(
        !app.history.can_redo(),
        "the redo must have actually fired, emptying the redo stack"
    );
}
...
fn ctrl_shift_c_fires_copy_all_and_never_also_the_plain_selection_copy_path() {
    ...
    assert!(
        app.internal_clipboard.is_none(),
        "sanity: nothing has been copied yet"
    );
...
fn validate_key_claims_reports_a_synthetic_collision_naming_both_claimants() {
    let claims = vec![
        KeyClaim {
            key: egui::Key::X,
            source: ClaimSource::Chord("Swap Colors"),
        },
        KeyClaim {
            key: egui::Key::X,
            source: ClaimSource::Tool("Sprinkler"),
        },
    ];
    let err = validate_key_claims(&claims).expect_err("a duplicate key claim must be an error");
    assert!(
        err.contains("Swap Colors"),
        "error must name the first claimant: {err}"
    );
    assert!(
        err.contains("Sprinkler"),
        "error must name the second claimant: {err}"
    );
}
...
fn ctrl_n_on_a_clean_document_opens_the_new_dialog_directly() {
    ...
    raw.events
        .push(key_event(egui::Key::N, egui::Modifiers::COMMAND));
    let _ = ctx.run_ui(raw, |ui| app.handle_keys(ui));

    assert_eq!(
        app.open_dialog,
        Some(OpenDialog::New),
        "Ctrl+N on a clean document must open the New dialog"
    );
    assert!(
        app.confirm.is_none(),
        "a clean document must not raise the unsaved-changes veto"
    );
}
...
fn ctrl_n_on_a_dirty_document_raises_the_unsaved_changes_confirm_instead_of_opening_new_directly() {
    ...
    assert!(
        app.is_dirty(),
        "sanity: the committed stroke made the document dirty"
    );
    ...
    assert_eq!(
        app.confirm,
        Some(PendingConfirm::NewDocument),
        "a dirty document must veto through the confirm"
    );
    assert_eq!(
        app.open_dialog, None,
        "the New dialog must not open directly while the veto is pending"
    );
}
...
fn g_toggles_the_grid_overlay_through_handle_keys_while_unfocused() {
    ...
    let mut raw = egui::RawInput::default();
    raw.events
        .push(key_event(egui::Key::G, egui::Modifiers::NONE));
    let _ = ctx.run_ui(raw, |ui| app.handle_keys(ui));
...
fn g_key_repeat_does_not_toggle_the_grid() {
    ...
    assert_eq!(
        app.show_grid, !starting,
        "sanity: the initial press toggles the grid"
    );
    ...
    assert_eq!(
        app.show_grid, !starting,
        "a held-down key-repeat G press must not toggle the grid again"
    );
}
...
fn question_mark_via_handle_keys_opens_the_help_overlay_while_unfocused() {
    ...
    let mut raw = egui::RawInput::default();
    raw.events
        .push(key_event(egui::Key::Questionmark, egui::Modifiers::NONE));
    let _ = ctx.run_ui(raw, |ui| app.handle_keys(ui));

    assert_eq!(
        app.open_dialog,
        Some(OpenDialog::Help),
        "? must open the overlay"
    );
}
...
fn plugin_ticks_suppressed_latches_while_a_modal_is_open_and_clears_on_the_next_real_tick() {
    let mut app = GasciiApp::headless();
    assert!(
        !app.plugin_ticks_suppressed,
        "sanity: a fresh app starts with nothing suppressed"
    );
    ...
    assert!(
        app.plugin_ticks_suppressed,
        "a modal-open frame must set the latch"
    );
    ...
    assert!(
        !app.plugin_ticks_suppressed,
        "the next real handle_keys call must clear the latch"
    );
}
...
fn help_overlay_renders_and_closes_on_escape_dismiss() {
    ...
    assert_eq!(
        app.open_dialog,
        Some(OpenDialog::Help),
        "sanity: a no-input frame must not close the overlay"
    );
    ...
    assert_eq!(
        app.open_dialog, None,
        "Escape must dismiss the overlay, matching every other dialog"
    );
}
...
fn the_help_overlay_lists_every_declared_plugin_shortcut() {
    let declared: Vec<(&str, &str)> = PLUGINS
        .iter()
        .flat_map(|d| (d.shortcuts)())
    ...
    assert_eq!(
        declared.len(),
        5,
        "4 gascii-anim rows + 1 gascii-density-brush row"
    );
}
...
fn ctrl_plus_requests_the_same_zoom_in_as_ctrl_equals() {
    ...
    raw.events
        .push(key_event(egui::Key::Plus, egui::Modifiers::COMMAND));
    let _ = ctx.run_ui(raw, |ui| app.handle_keys(ui));

    assert_eq!(
        app.pending_step_zoom, 1,
        "Ctrl+Plus must request the same one-step zoom-in as Ctrl+="
    );
}
...
fn ctrl_a_via_handle_keys_rebinds_l_to_selection_and_selects_the_whole_document_by_default() {
    ...
    assert_eq!(
        app.slot(Binding::L).kind,
        ToolKind::Selection,
        "Ctrl+A must rebind L by default"
    );
    assert_eq!(app.selection_slot(), Some(Binding::L));
    assert_eq!(
        app.slot(Binding::L)
...
fn ctrl_a_via_handle_keys_prefers_a_binding_that_already_holds_selection() {
    ...
    assert_eq!(
        app.slot(Binding::L).kind,
        ToolKind::Pencil,
        "L must be left untouched"
    );
    assert_eq!(
        app.selection_slot(),
        Some(Binding::R),
        "Ctrl+A must select through R, which already held it"
    );
}
...
fn ctrl_a_is_suppressed_while_a_widget_has_keyboard_focus() {
    ...
    assert_eq!(
        app.slot(Binding::L).kind,
        ToolKind::Pencil,
        "a focused widget must suppress Ctrl+A"
    );
}
...
fn ctrl_a_during_a_live_text_burst_commits_the_burst_before_switching_to_select_all() {
    ...
    assert_eq!(
        app.doc.cell(0, 0, 0).unwrap().ch,
        'h',
        "the burst's typed text must be committed, not discarded"
    );
    assert_eq!(
        app.doc.cell(0, 1, 0).unwrap().ch,
        'i',
        "the burst's typed text must be committed, not discarded"
    );
    assert_eq!(
        app.slot(Binding::L).kind,
        ToolKind::Selection,
        "Ctrl+A must switch the Text binding to Selection"
    );
    assert_eq!(
        app.slot(Binding::L)
            .tool
...
fn duplicate_selection_commits_a_copy_and_leaves_the_original_in_place() {
    ...
    let b = app
        .selection_slot()
        .expect("the selection session survives a duplicate");
    let float_rect = app
        .slot(b)
        .tool
        .selection_overlay()
        .and_then(|v| v.marquee)
        .expect("the copy is a live float");
    assert_eq!(
        (float_rect.x0, float_rect.y0),
        (1, 1),
        "the copy lands one cell down-right of the source"
    );
    ...
    assert_eq!(
        app.doc.cell(0, 1, 1).unwrap().ch,
        'A',
        "the committed copy lands at the offset"
    );
}
...
fn recolor_selection_with_a_transparent_bg_well_recolors_glyphs_and_keeps_backgrounds() {
    ...
    assert_eq!(
        app.active_bg,
        Rgba::TRANSPARENT,
        "sanity: the BG well starts transparent"
    );
    ...
    assert_eq!(
        recolored.fg,
        Rgba(200, 0, 0, 255),
        "the glyph's text color follows the active FG"
    );
    assert_eq!(recolored.ch, 'A', "the glyph itself is untouched");
    assert_eq!(
        recolored.bg, kept_bg,
        "a transparent BG well must never wipe an existing background"
    );
    assert_eq!(
        app.doc.cell(0, 0, 0),
        Some(&gascii_core::Cell::BLANK),
    ...
    app.request_undo();
    assert_eq!(
        app.doc.cell(0, 1, 1).unwrap().fg,
        old_fg,
        "undo restores the previous text color"
    );
}
...
fn recolor_selection_with_a_set_bg_well_recolors_backgrounds_of_painted_cells_only() {
    ...
    assert_eq!(
        glyph_cell.fg,
        Rgba(200, 0, 0, 255),
        "the glyph's text color follows the FG well"
    );
    assert_eq!(
        glyph_cell.bg,
        Rgba(0, 0, 200, 255),
        "the glyph cell's background follows the BG well"
    );
    let bg_only = app.doc.cell(0, 2, 1).unwrap();
    assert_eq!(
        bg_only.bg,
        Rgba(0, 0, 200, 255),
        "a bg-only cell's background follows the BG well too"
    );
    assert_eq!(
        bg_only.fg, old_fg,
        "a space glyph's text color is meaningless and left alone"
    );
    assert_eq!(
        app.doc.cell(0, 0, 0),
        Some(&gascii_core::Cell::BLANK),
        "blank cells are never filled"
    );

    app.request_undo();
    assert_eq!(
        app.doc.cell(0, 1, 1).unwrap().bg,
        old_bg,
...
fn bare_d_key_is_bound_to_nothing_and_leaves_colors_and_tools_untouched() {
    ...
    let mut raw = egui::RawInput::default();
    raw.events
        .push(key_event(egui::Key::D, egui::Modifiers::NONE));
    let _ = ctx.run_ui(raw, |ui| app.handle_keys(ui));

    assert_eq!(
        app.active_fg, fg_before,
        "bare D must not touch the active foreground color"
    );
    assert_eq!(
        app.active_bg, bg_before,
        "bare D must not touch the active background color"
    );
    assert_eq!(
        app.slot(Binding::L).kind,
        ToolKind::Pencil,
        "bare D must not rebind L"
    );
    assert_eq!(
        app.slot(Binding::R).kind,
        ToolKind::Eraser,
        "bare D must not rebind R"
    );
}
...
fn ctrl_x_via_handle_keys_copies_and_deletes_the_selection_in_one_change() {
    ...
    assert_eq!(
        app.doc.cell(0, 1, 1).unwrap().ch,
        ' ',
        "Ctrl+X must also delete the selected cell"
    );
}
...
fn ctrl_x_via_handle_keys_is_a_no_op_without_a_live_selection() {
    ...
    assert_eq!(
        app.doc.cell(0, 1, 1).unwrap().ch,
        'x',
        "no selection: nothing may be deleted"
    );
}
...
fn ctrl_d_via_handle_keys_duplicates_the_selection_as_a_moveable_float() {
    ...
    assert_eq!(
        app.selection_slot(),
        Some(Binding::L),
        "sanity: L holds the live selection"
    );
    ...
    assert_eq!(
        app.keyboard_owner(),
        Some(Binding::L),
        "the duplicate is a live session — keyboard stays claimed"
    );
    let float_rect = app
        .slot(Binding::L)
        .tool
    ...
    assert_eq!(
        app.doc.cell(0, 2, 2).unwrap().ch,
        'x',
        "dropping the float commits the copy"
    );
    assert_eq!(
        app.doc.cell(0, 1, 1).unwrap().ch,
        'x',
        "the original survives the drop too"
    );
}
...
fn ctrl_d_via_handle_keys_is_a_no_op_without_a_live_selection() {
    ...
    assert_eq!(
        app.slot(Binding::L).kind,
        ToolKind::Pencil,
        "nothing may change with no live selection"
    );
}
...
fn ctrl_d_is_suppressed_while_a_widget_has_keyboard_focus() {
    ...
    assert_eq!(
        app.keyboard_owner(),
        Some(Binding::L),
        "a focused widget must suppress Ctrl+D's keyboard release"
    );
}
...
fn plugin_tick_panel_outcome_edits_reach_apply_edit_via_handle_keys() {
    ...
    raw.events
        .push(key_event(egui::Key::D, egui::Modifiers::SHIFT));
    let _ = ctx.run_ui(raw, |ui| app.handle_keys(ui));
...
fn plugin_tick_panel_outcome_set_active_frame_reaches_switch_active_frame_via_handle_keys() {
    ...
    let mut raw = egui::RawInput::default();
    raw.events
        .push(key_event(egui::Key::Period, egui::Modifiers::NONE));
    let _ = ctx.run_ui(raw, |ui| app.handle_keys(ui));
...
fn comma_period_and_shift_d_are_suppressed_while_a_widget_has_keyboard_focus() {
    ...
    raw.events
        .push(key_event(egui::Key::Comma, egui::Modifiers::NONE));
    raw.events
        .push(key_event(egui::Key::D, egui::Modifiers::SHIFT));
    let _ = ctx.run_ui(raw, |ui| {
        ui.memory_mut(|m| m.request_focus(egui::Id::new("qa_test_fake_focused_widget")));
    ...
    assert_eq!(
        app.doc.frame_count(),
        before_frame_count,
        "a focused widget must suppress Shift+D"
    );
}
